...
import GrabCoinsGame from "@/components/games/GrabCoinsGame";

export const dynamic = "force-dynamic";

export default async function GrabCoinsPage({
    params,
}: {
    ...
    if (!gameConfig.grab_coins) {
        return (
            <GameUnavailable
                slug={slug}
                message="Grab the Coins is not enabled for this event."
            />
        );
    }
    ...
    return (
        <main className="flex min-h-[100dvh] items-center justify-center bg-[#F7F5FF] px-4 py-8 text-slate-950">
            <section className="w-full max-w-lg rounded-[1.5rem] border border-slate-200 bg-white p-6 text-center shadow-sm md:rounded-[2rem] md:p-8">
                <h1 className="text-2xl font-black">Grab the Coins unavailable</h1>
